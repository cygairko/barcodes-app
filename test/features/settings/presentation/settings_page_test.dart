...
Future<void> pumpSettingsPage(
  ...
  when(mockSettingsRepository.getAutomaticScreenBrightness())
      .thenAnswer((_) async => initialAutoBrightness);
  when(mockSettingsRepository.getMaxScreenBrightnessLevel())
      .thenAnswer((_) async => initialMaxBrightnessLevel);

  // Mock the set methods to do nothing or return a completed Future
  when(mockSettingsRepository.setAutomaticScreenBrightness(any))
      .thenAnswer((_) async {});
  when(mockSettingsRepository.setMaxScreenBrightnessLevel(any))
      .thenAnswer((_) async {});
...
void main() {
  ...
  group('SettingsPage Widget Tests', () {
    testWidgets('Initial state reflects provider values and slider is disabled',
      ...
      // Verify switch state
      final SwitchListTile switchTile =
          tester.widget(find.byKey(const Key('automaticBrightnessSwitch')));
      expect(switchTile.value, isFalse);

      // Verify slider value
      final Slider slider =
          tester.widget(find.byKey(const Key('maxBrightnessSlider')));
      expect(slider.value, 0.7);

      // Verify slider is disabled (onChanged is null)
      expect(slider.onChanged, isNull);
      expect(find.text('Enable automatic brightness to set level.'),
          findsOneWidget);
    });

    testWidgets(
        'Tapping automatic brightness switch calls repository and updates UI',
        (WidgetTester tester) async {
      ...
      SwitchListTile switchTile =
          tester.widget(find.byKey(const Key('automaticBrightnessSwitch')));
      expect(switchTile.value, isFalse,
          reason: "Switch should initially be off");
      Slider slider =
          tester.widget(find.byKey(const Key('maxBrightnessSlider')));
      expect(slider.onChanged, isNull,
          reason: "Slider should initially be disabled");
      ...
      verify(mockSettingsRepository.setAutomaticScreenBrightness(true))
          .called(1);
      ...
      expect(
        ...
        isTrue,
        reason:
            "Switch should be on after tap. Provider may not have updated UI if this fails.",
      );
      expect(slider.onChanged, isNotNull,
          reason: "Slider should be enabled when switch is on");
      expect(
          find.text('Enable automatic brightness to set level.'), findsNothing);
    });

    testWidgets('Changing slider value calls repository when switch is on',
        (WidgetTester tester) async {
      ...
      final double newSliderValue =
          0.9; // The value we expect the slider to move to visually
      when(mockSettingsRepository.getMaxScreenBrightnessLevel())
          .thenAnswer((_) async => newSliderValue);
      ...
      await tester.drag(find.byKey(const Key('maxBrightnessSlider')),
      ...
      final captured =
          verify(mockSettingsRepository.setMaxScreenBrightnessLevel(captureAny))
              .captured;
      expect(captured, isNotEmpty,
          reason: "setMaxScreenBrightnessLevel should be called by the drag");
      ...
      slider = tester.widget(find.byKey(const Key('maxBrightnessSlider')));
      expect(slider.value, newSliderValue,
          reason:
              "Slider value should update after interaction and provider refresh");
    });

    testWidgets(
        'Slider is disabled and does not call repository when switch is off',
        (WidgetTester tester) async {
      // Initial state: switch is OFF
      when(mockSettingsRepository.getAutomaticScreenBrightness())
          .thenAnswer((_) async => false);
